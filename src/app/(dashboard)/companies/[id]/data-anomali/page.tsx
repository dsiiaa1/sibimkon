...
export default function DataAnomaliPage() {
  ...
  const loadEntries = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          bimkon_projects!inner(
            title,
            company_id,
            companies(name)
          )
        `)
        .eq('bimkon_projects.company_id', companyId)
      ...
      const mapped: AuditLogEntry[] = (data ?? []).map((row: any) => {
        const proj = row.bimkon_projects
        const comp = proj?.companies
        // Handle array vs object for companies (PostgREST sometimes returns array for one-to-many although it's many-to-one here)
        const compName = Array.isArray(comp) ? comp[0]?.name : comp?.name
        
        return {
          ...row as AuditLogEntry,
          project_title: proj?.title ?? '—',
          company_name: compName ?? 'Lainnya (Tanpa Perusahaan)',
        }
      })

      setEntries(mapped)

      // Hitung statistik khusus perusahaan ini
      const { data: allStats } = await supabase
        .from('data_audit_log')
        .select(`
          status,
          bimkon_projects!inner(company_id)
        `)
        .eq('bimkon_projects.company_id', companyId)
      ...
      console.error('[data-anomali] Error:', err)
    } finally {
      setLoading(false)
      setRefreshing(false)
    }
  ...
  const handleReview = async (entry: AuditLogEntry, action: 'approved' | 'rejected') => {
    ...
    try {
      ...
      await showAlert(`Gagal memperbarui status: ${msg}`, 'Error')
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-3 bg-slate-950/40 p-4 rounded-2xl border border-slate-800/80">
        ...
        <span className="text-xs text-slate-500 font-medium uppercase tracking-wider">Filter Status:</span>
        ...
        <div className="space-y-6">
          {Object.entries(
            entries.reduce((acc, entry) => {
              const cName = entry.company_name || 'Lainnya (Tanpa Perusahaan)';
              if (!acc[cName]) acc[cName] = [];
              acc[cName].push(entry);
              return acc;
            }, {} as Record<string, AuditLogEntry[]>)
          ).map(([companyName, companyEntries]) => (
            <div key={companyName} className="space-y-3">
              <h2 className="text-lg font-bold text-slate-200 border-b border-slate-800/80 pb-2 mb-3">
                🏢 {companyName}
              </h2>
              {companyEntries.map(entry => {
                const sc = STATUS_CONFIG[entry.status]
                const isExpanded = expandedId === entry.id
                const ruleLabel = RULE_LABELS[entry.validation_rule_failed] ?? entry.validation_rule_failed

                return (
                  <div
                    key={entry.id}
                    className={`rounded-2xl border transition-all ${sc.bg} ${sc.border}`}
                  >
                    {/* Row utama */}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-800 border border-slate-700 text-slate-400">
                            <Database className="h-3 w-3" />{ruleLabel}
                          </span>
                          {/* Source table */}
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-1.5">
                          <span className="text-[11px] text-slate-500">
                            Proyek: <span className="text-slate-400">{entry.project_title}</span>
                          </span>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 border-t border-slate-800/50 pt-3">
                        <div className="grid grid-cols-2 gap-3 text-xs">
                          ...
                          {entry.reviewed_at && (
                            <div className="col-span-2">
                              <p className="text-slate-500 font-medium mb-1">Direview pada</p>
                              <p className="text-slate-300">{new Date(entry.reviewed_at).toLocaleString('id-ID')}</p>
                            </div>
                          )}
                        </div>
                      </div>
